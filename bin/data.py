...
class Data(object):
    ...
    def get_permissions():
        ...
        fname = os.path.join(webiap.config.CONFIG_DIR, "users.csv")
        tools.touch(fname)
        perm = {}
    ...
    def make_incidence(self):
        ...
        for country in self.countries:
            orig_name = os.path.join(webiap.config.DIRS["csv"],
                                     "{0}_all.csv".format(country))
            if not os.path.exists(orig_name):
                continue

            new_name = os.path.join(webiap.config.DIRS["download"], country,
                                    "incidence",
                                    "{0}_all.csv".format(country))
    ...
    def make_geo(self):
        ...
        for period in self.periods:
            ...
            new_name = os.path.join(
                webiap.config.DIRS["download"], country, "geo",
                "{0}.zip".format(period))
            tools.create_dir(new_name, remove=True)
            os.symlink(orig_name, new_name)
        ...
        for period in self.periods:
            ...
            for fname in questions:
                if fname.startswith(src) or fname.startswith(src2):
                    ...
                    new_name = os.path.join(
                        webiap.config.DIRS["download"], country, "orig", fname)
                    tools.create_dir(new_name, remove=True)
                    os.symlink(orig_name, new_name)
...
if __name__ == "__main__":
    my_htpass = HtPass(os.path.join(webiap.config.CONFIG_DIR, "htpasswd.txt"))
    data = Data(my_htpass)
    data.create_users()
    data.make_incidence()
    data.make_converted()
