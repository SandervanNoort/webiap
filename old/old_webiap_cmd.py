...
class Website(object):
    ...
    def parse(self, name):
        ...
        if name in ["cold", "allergy", "gastro"]:
            return [name + ili[3:] for ili in self.parse("ili")]
    ...
    #         all_values.extend(
    #             webiap_old.config.DIAGRAMS["params"]["casedef_list"])
    ...
    def get_run(self):
        ...
        if self.cmd_options.cpus > 1:
            self.pool = tools.Pool(self.cmd_options.cpus)

            # output is written to stdout
            self.pool.add_listener(sys.stdout)
    ...
    def main(self):
        ...
        try:
            ...
            for (options["lang"],
                ...
                for options["source"] in self.parse("source"):
...
#                     continue
                ...
                for options["casedef_id"] in (
                        self.parse("casedef") + self.parse("syndrome")):
                ...
                for options["activity"] in self.parse("activity"):
                    run(webiap.Activity, options)

                for options["cutter"] in self.parse("histogram"):
                    run(webiap.HistCutter, options)

                for options["casedef_id"] in self.parse("ili"):
                    for options["analysis"] in self.parse("analysis"):
                        run(webiap.Analysis, options)
                    for options["compare"] in self.parse("compare"):
                        run(webiap.Compare, options)
                    for options["age"] in self.parse("age"):
                        run(webiap.Age, options)

                    for options["cutter"], options["average"] in \
                            itertools.product(self.parse("cutter"),
                                              self.parse("average")):
                        run(webiap.IliCutter, options)

                    for options["control_id"] in self.parse("control"):
                        run(webiap.Control, options)

                if self.cmd_options.symptoms:
                    for options["casedef_id"] in (
                            self.parse("syndrome") + self.parse("venn")):
