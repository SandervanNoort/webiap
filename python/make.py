...
class Website(object):
    ...
    def parse(self, name):
        ...
        all_values = tools.SetList(
            ifig.config.DIAGRAMS["params"][name + "_list"])

    #     elif name == "week_casedef_list":
    #         all_values.extend(ifig.config.DIAGRAMS["params"]["ili_list"])
    #         all_values.extend(ifig.config.DIAGRAMS["params"]["cold_list"])
    #         all_values.extend(ifig.config.DIAGRAMS["params"]["venn_list"])
    #         all_values.extend(ifig.config.DIAGRAMS["params"]["casedef_list"])
    #     elif name == "week_cutter_list":
    #         all_values.extend(ifig.config.DIAGRAMS["params"]["cutter_list"])

        default = (ifig.config.DIAGRAMS["params"][name + "_default"]
                   if name + "_default" in ifig.config.DIAGRAMS["params"] else
                   [])
    ...
    def get_cmd(self):
        ...
        if cmd_options.reload:
            ifig.config.DIAGRAMS["ini"]["reload"] = "True"
    ...
    def main(self):
        ...
        try:
            ...
            for (options["lang"],
                ...
                #     ifig.config.DIAGRAMS["ini"]["plot_start"] = ""
                #     ifig.config.DIAGRAMS["ini"]["plot_end"] = ""
                #     ifig.config.DIAGRAMS["ini"]["plot_start_year"] = ""
                #     ifig.config.DIAGRAMS["ini"]["plot_end_year"] = ""
                # if options["season"] in ["2012/13"]:
                #     ifig.config.DIAGRAMS["ini"]["plot_end"] = "6/1"
                #     ifig.config.DIAGRAMS["ini"]["plot_end_year"] = "2013/6/1"
                # if options["country"] in ["br"]:
                #     ifig.config.DIAGRAMS["ini"]["min_value"] = 10
                #     ifig.config.DIAGRAMS["ini"]["min_participants"] = 10
                #     ifig.config.DIAGRAMS["ini"]["min_surveys"] = 1

                for source in self.parse("source"):
                    run(ifig.Europe, dict(options, source=source))

#                 if options["country"] not in ifig.config.DIAGRAMS[
#                       "params"]["inet"]:
#                     continue

                for casedef_id in (self.parse("casedef") +
                                   self.parse("syndrome")):
                    run(ifig.Casedef, dict(options, casedef_id=casedef_id))
                ...
                for activity in self.parse("activity"):
                    run(ifig.Activity, dict(options, activity=activity))

                for cutter in self.parse("histogram"):
                    run(ifig.HistCutter, dict(options, cutter=cutter))
                ...
                for options["casedef_id"] in self.parse("ili"):
                    ...
                    for compare in self.parse("compare"):
                        run(ifig.Compare, dict(options, compare=compare))
                    for age in self.parse("age"):
                        run(ifig.Age, dict(options, age=age))
            ...
            if "all" in times["seasons"] and self.cmd_options.csv:
                ...
                for (options["country"], options["subset"]) \
                    ...
                    if (options["country"]
                            not in ifig.config.DIAGRAMS["params"]["inet"]):
                        continue
                    run(ifig.Csv, options)
